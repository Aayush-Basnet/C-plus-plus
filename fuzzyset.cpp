...
// input of set datas 
 ...
 cin>>n2; 
 cout<<"Enter the members of set a with its members"<<endl;  for(int i=0;i<n1;i++){ 
 cin>>a[i]; 
 cin>>m[i]; 
 } 
 cout<<"A = "; 
display(a,m,n1); 
 ...
 cout<<"B = "; 
display(b,n,n2); 
 //finding intersection of set a and b 
 for(int i=0;i<n1;i++){ 
 for(int J=0;J<n2;J++) 
 if(a[i]==b[J]){ 
 c[k]=a[i]; 
 ...
 cout<<endl<<"INTERSECTION =";  display(c,me,d); 
for (int i = 0 ; i<n1 ; i++){ 
p[i]=a[i]; 
te[i]=m[i]; 
} 
...
cout<<endl <<"COMPLEMENT OF A = "; display(a,cm,n1); 
cout<<endl <<"COMPLEMENT OF B = "; display(b,cn,n2); 
 return 0; 
}
